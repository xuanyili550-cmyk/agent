"""DramaBox 发布客户端：GenericHTTPAdapter 的薄封装，只负责定位配置文件。

DramaBox 没有公开开发者 API，所以端点/请求体全靠 config.yaml；配置查找顺序：
显式参数 > DRAMABOX_CONFIG_PATH 环境变量 > 同目录 config.yaml > config.example.yaml（示例里端点是 REPLACE_WITH 占位，
...
class DramaBoxPublishClient(GenericHTTPAdapter):
    """见 README.md：DramaBox 没有公开开发者 API。这是一个可配置的 HTTP 骨架，不是真正的接入实现。"""
