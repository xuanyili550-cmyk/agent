"""
================================================================================
 Deep RL Course · Chapter 4 · 策略梯度(REINFORCE 手写)（学习笔记描述）
================================================================================
 一句话：不学价值、直接学策略——策略梯度让"拿到高回报的动作"概率变大(REINFORCE)。
 本章讲：
   ① 策略网络输出动作分布(Categorical)，采样动作。
   ② 用整条轨迹的回报做梯度上升(REINFORCE)。
   ③ 在 CartPole/PixelCopter 上从零实现训练循环。
 要点：值方法(DQN) vs 策略方法(PG)的分野；PG 适合连续/随机策略。
 说明：notebook 摘录(含 %matplotlib inline，非法纯 .py)，需 torch/gym；参考为主，不直接跑。
================================================================================
"""
