"""
================================================================================
 CV Course · Chapter 11 · 零样本学习(ZSL)（学习笔记描述）
================================================================================
 一句话：识别"训练时没见过的类别"——靠语义嵌入把"没见过的类"和"见过的知识"连起来。
 本章讲：
   ① ZSL vs GZSL(测试是否含已见类，后者更贴近现实、更难)。
   ② 归纳式(训练完全不接触未见类) vs 直推式(可用未见类的无标注信息)。
   ③ 用语义嵌入(属性/词向量/CLIP)实现 ZSL。
 要点：ZSL 的桥梁是"共享的语义空间"(如 CLIP 的图文对齐)。
 说明：概念 + 示例(语义嵌入)，参考为主。
================================================================================
"""
