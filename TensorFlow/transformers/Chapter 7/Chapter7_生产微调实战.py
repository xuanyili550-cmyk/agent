"""
================================================================================
 Chapter 7 · 生产级微调流水线（真实数据 + 真实模型 + 完整训练工程）
================================================================================
 这是“能写进简历/能上线”的版本，不是玩具：
   · 真实数据集 fancyzhx/ag_news(4 类新闻)、真实模型 distilbert，不是合成数据/2层小模型
   · 完整训练工程：Accelerate 训练循环 + 学习率warmup调度 + 梯度累积 + 梯度裁剪 +
     混合精度 + 参数分组权重衰减 + 每轮评估(准确率+宏F1) + 最优检查点 + 早停 + 保存 + 断点续训
   · 配置集中(dataclass)、可复现(set_seed)、结构化日志、训练完直接做推理演示

 这套“训练骨架”适用于所有任务——换任务只需改 3 处：
     模型类(AutoModelForXxx) + 数据整理器(DataCollator) + 评估指标。
   NER → AutoModelForTokenClassification + DataCollatorForTokenClassification + seqeval
   QA  → AutoModelForQuestionAnswering + default_data_collator + squad(EM/F1)
   摘要/翻译 → AutoModelForSeq2SeqLM + DataCollatorForSeq2Seq + ROUGE/BLEU

 用法：
   python3 Chapter7_生产微调实战.py                 # 快速真训练(小子集，~1-2 分钟，会真出指标)
   python3 Chapter7_生产微调实战.py --epochs 3 --subset 5000   # 训久一点、数据更多
   python3 Chapter7_生产微调实战.py --full          # 全量数据(慢，演示生产规模)
 训练完自动保存到 ./ag_news-distilbert/ 并做几条推理演示。
================================================================================
"""
...
# ==============================================================================
# 指标（自实现，不依赖 sklearn/seqeval —— 生产里也常自己算保证可控）
# ==============================================================================
def compute_metrics(preds, labels, num_classes):
    preds, labels = np.asarray(preds), np.asarray(labels)
    acc = float((preds == labels).mean())
    # 宏 F1：每类各算 F1 再平均(对类别不均衡更公平)
    f1s = []
...
def build_dataloaders(cfg, tokenizer):
    ...
    if cfg.subset_train:                       # 快速模式取子集；生产全量就把它设 0
    ...
    train.set_format("torch"); test.set_format("torch")
...
# ==============================================================================
# 优化器参数分组（bias / LayerNorm 不做权重衰减，这是标准做法）
# ==============================================================================
...
def train(cfg: TrainConfig):
    ...
    set_seed(cfg.seed)                                       # 可复现
    accelerator = Accelerator()                             # 自动选设备(cuda/mps/cpu)+混合精度
    accelerator.print(f">>> 设备={accelerator.device}  数据集={cfg.dataset_name}")
    ...
    for epoch in range(cfg.epochs):
        ...
        if metrics["macro_f1"] > best_f1:
            ...
            if no_improve >= cfg.patience:
                accelerator.print(f"    ⏹ 早停：macro_f1 连续 {cfg.patience} 轮没提升"); break
...
# ==============================================================================
# 推理演示（加载保存好的模型，像生产上线那样用）
# ==============================================================================
def inference_demo(cfg: TrainConfig):
    from transformers import AutoTokenizer, AutoModelForSequenceClassification
    if not os.path.isdir(cfg.output_dir):
        print("(还没训练出模型，跳过推理演示)"); return
...
def parse_args():
    ...
    p.add_argument("--subset", type=int, default=3000, help="训练子集大小；0/--full 表示全量")
    p.add_argument("--full", action="store_true", help="用全量数据(慢)")
    return p.parse_args()
...
if __name__ == "__main__":
    ...
    cfg = TrainConfig(epochs=args.epochs,
                      subset_train=0 if args.full else args.subset,
                      subset_eval=0 if args.full else 800)
    train(cfg)
    inference_demo(cfg)
