...
def case_03():
    ...
    trainer=Trainer(model,args,train_dataset=small_train,eval_dataset=small_eval,data_collator=collator,processing_class=tok)
...
# ---- 微调：小子集快速训练 ----
    raw = load_dataset("nyu-mll/glue", "mrpc")
    tokenized =raw.map(lambda e : tok(list(e["sentence1"]), list(e["sentence2"]), truncation=True), batched=True)
    collator=DataCollatorWithPadding(tokenizer=tok)
...
def run_choice(choice: str):
    ...
    if choice == "all":
        for n in sorted(CASES):
            CASES[n]()
        return
    # 先只判断“关号合不合法”，不要把 case 的执行也包进 try，
    # 否则 case 内部的 KeyError/ValueError 会被误吞成“没有第 N 关”，掩盖真正的报错。
    try:
        n = int(choice)
    except ValueError:
    ...
    if n not in CASES:
        print(f"没有第 {choice} 关，请输入 1~6 或 all。")
        return
    CASES[n]()   # 单独调用：case 内部真报错会正常抛出 traceback，不再被掩盖
