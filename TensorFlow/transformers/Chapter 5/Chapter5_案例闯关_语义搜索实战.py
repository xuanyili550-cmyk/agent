"""
================================================================================
 Chapter 5 案例闯关 · 数据集处理 + 语义搜索（6 关，全部能在 Mac 上跑）
================================================================================
 用法：
   python3 Chapter5_案例闯关_语义搜索实战.py         # 看菜单
   python3 Chapter5_案例闯关_语义搜索实战.py 3        # 只跑第 3 关
   python3 Chapter5_案例闯关_语义搜索实战.py all       # 全部
 也可在 PyCharm 直接点运行 → 按提示输入关号。

 关卡地图（3~6 关是语义搜索，RAG 检索核心）：
   1  数据集处理三招     from_dict + filter + map + rename（不下大数据集）
   2  文本嵌入          把一句话变成 768 维向量，看 shape
   3  手动语义搜索       嵌入语料+问题 → 点积 → top-k（不用 faiss，看原理）
   4  FAISS 索引检索     datasets 内置 faiss：add_faiss_index + get_nearest_examples
   5  ★语义 vs 关键词    换个说法也能搜到，关键词却搜不到
   6  综合:迷你搜索引擎   一个小知识库，多条问题检索（RAG 检索雏形）

 说明：用自包含小语料，几秒跑完；嵌入模型首次会下载(~420MB)一次。
================================================================================
"""
...
# 公共：加载嵌入模型 + 定义 get_embeddings（CLS 池化）
def load_embedder():
    ...
    def get_embeddings(text_list):
        ...
        with torch.no_grad():
            out = model(**enc)
        return out.last_hidden_state[:, 0]   # CLS 池化：取 [CLS] 向量代表整句
...
# ==============================================================================
# 第 3 关：手动语义搜索（不用 faiss，看原理）
# ==============================================================================
def case_03():
    title(3, "手动语义搜索：嵌入 → 点积 → top-k")
    tok, model, get_embeddings = load_embedder()

    corpus_emb = get_embeddings(CORPUS)              # [10, 768] 所有文档的向量
    query = "How do I use a dataset without internet?"
    q_emb = get_embeddings([query])                  # [1, 768] 问题的向量

    # 点积相似度（这个模型是 dot 训练的，越大越相似）
    scores = (q_emb @ corpus_emb.T)[0]               # [10]
...
# ==============================================================================
# 第 4 关：FAISS 索引检索（datasets 内置 faiss）
# ==============================================================================
def case_04():
    ...
    # 建一个带 text 列的数据集，map 出 embeddings 列
...
def case_05():
    ...
    # 用一个和目标文档"没有任何共同关键词"的问法
    query = "process data that does not fit in RAM"   # 目标: streaming...larger than memory
    print(f"问题：{query}")
