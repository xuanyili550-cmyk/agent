...
from .routers import health, moderate


@asynccontextmanager
async def lifespan(app):
    s = get_settings(); setup_logging(s.debug)
    get_logger("startup").info("启动: risk_backend=%s", s.risk_backend); yield
...
def create_app():
    ...
    app.include_router(health.router); app.include_router(moderate.router)
    return app
