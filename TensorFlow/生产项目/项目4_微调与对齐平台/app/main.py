...
from .routers import health, train
@asynccontextmanager
async def lifespan(app):
    s = get_settings(); setup_logging(s.debug)
    get_logger("startup").info("启动: train_backend=%s", s.train_backend); yield
def create_app():
    ...
    app.include_router(health.router); app.include_router(train.router); return app
app = create_app()
