...
"""
Gemma 4 (E2B 变体) 逐层调试对比脚本 / Gemma 4 (E2B variant) layer-by-layer debugging comparator.
...
def _import_gemma4_classes():
    ...
    try:
        ...
        # parents[4]：从当前文件 tests/test_e2b/gemma4_e2b_layer_debugger.py 往上数 4 级目录，
...
def tiny_debug_config():
    """
    构造一份"迷你版" Gemma4 配置字典，专门用于调试对比，而非真实训练/推理规模。
...
def load_notebook_defs(nb_name="standalone-gemma4-plus-kvcache.ipynb"):
    """
    从相邻目录下的 Jupyter Notebook（默认是 `standalone-gemma4-plus-kvcache.ipynb`）中
    动态导入其中定义的类/函数（例如 Gemma4DenseModel、load_weights_into_gemma4_dense 等），
    这样就不必把 notebook 里的代码手动复制一份到 .py 文件里维护两份。

    parents[2]：从当前文件 tests/test_e2b/xxx.py 往上数 2 级目录，得到
...
def _attach_debug_hooks(model, is_hf):
    ...
    def scaled_embedding_hook(name, scale):
        ...
        原因：Gemma 系列模型的做法是"词嵌入输出 = nn.Embedding(input_ids) * sqrt(hidden_size)"，
        但在 "ours" 的 Gemma4DenseModel 实现里，这个乘以 sqrt(emb_dim) 的缩放是写在
        forward() 方法里 `self.tok_emb(input_ids) * (self.cfg["emb_dim"] ** 0.5)` 这一行，
        并不在 self.tok_emb 这个子模块自己的 forward 内部完成。
        所以如果直接对 self.tok_emb 挂普通 hook，拿到的只是"未缩放"的原始嵌入，
        无法代表真正流入第一个 transformer block 的数值。这里手动在 hook 里补上同样的缩放，
        使得记录下来的 "embedding" trace 能对应上模型实际使用的（已缩放）嵌入值。

        对照 HF 官方实现：Gemma4TextScaledWordEmbedding.forward 内部就是
        `return super().forward(input_ids) * self.embed_scale`，缩放是在子模块自己
        forward 内部完成的，所以对 HF 的 embed_tokens 直接用普通 hook（见下方 `hook("embedding")`）
        取到的输出，本身就已经是"缩放后"的结果——这样两边 "embedding" 这一条 trace
        才是同一含义、可以直接比较的量。（已通过实际运行验证：两边完全一致，max diff = 0。）
        """
        def _record(_, __, output):
            if isinstance(output, tuple):
                output = output[0]
