...
"""
模块说明（中文）：
本文件是第 5 章 "Pretraining on Unlabeled Data"（在无标签数据上进行预训练）主代码的一个变体，
位于 ch05/18_muon 目录下，用途是作为 **Muon 优化器对比实验中的基线（baseline）训练脚本**：
全程使用标准的 AdamW 优化器训练 GPT 模型，训练流程、超参数与评估方式都与
同目录下的 gpt_train_muon.py（使用 Muon + AdamW 混合优化器）保持一致，
唯一的差异就是优化器本身，便于对照两者的收敛速度/最终损失，从而评估 Muon 优化器的效果。
...
def train_model_simple(model, train_loader, val_loader, optimizer, device, num_epochs,
    ...
    本函数是 AdamW 基线与 gpt_train_muon.py 中 Muon 版本对比的核心：两者的训练循环
    结构完全一致，唯一区别是这里的 optimizer 只有一个（AdamW），而 Muon 版本
    需要同时 zero_grad/step 多个优化器（Muon 优化二维权重矩阵，AdamW 处理其余参数）。

    参数:
        model (torch.nn.Module): 待训练的 GPT 模型。
        train_loader (DataLoader): 训练集数据加载器。
        val_loader (DataLoader): 验证集数据加载器。
        optimizer (torch.optim.Optimizer): 优化器（此处固定为 AdamW）。
        device (torch.device): 计算设备。
...
def main(gpt_config, settings):
    """训练流程的总入口：下载/加载数据 -> 初始化模型与 AdamW 优化器 -> 构建数据加载器 -> 执行训练。

    这是 Muon 对比实验中的基线版本：优化器固定为 torch.optim.AdamW，
    对照版本见 gpt_train_muon.py 中的 main()（使用 create_muon_optimizers 构建
    Muon + AdamW 混合优化器）。除优化器构造这一处外，其余流程与对照版本保持一致，
    以保证对比的公平性。
    ...
    # 固定随机种子，保证模型参数初始化、Dropout 掩码、数据打乱顺序等在 CPU 上具有可复现性
    # 【风险提示，不修改】：若 device 为 "cuda"，仅设置 torch.manual_seed 并不能保证
    # cuDNN 等 GPU kernel 的计算完全确定性复现（还需要额外设置
    # torch.backends.cudnn.deterministic=True 等，且可能影响性能），这是 PyTorch/CUDA
    # 跨版本、跨硬件的已知行为，不属于本文件的确定性 bug，此处只标注、不改动。
    ...
    # 基线优化器：单一 AdamW 优化模型的全部参数（对照 Muon 版本会把二维权重矩阵
    # 交给 Muon 优化器，其余参数——如 Embedding、LayerNorm、bias 等——仍交给 AdamW）
    ...
    # 调用前面定义的训练循环，实际执行多轮梯度下降训练（基线：全程只用 AdamW）
...
if __name__ == "__main__":
    ...
    # 这是 GPT-2 "124M"（约 1.24 亿参数）规模的标准配置，与 gpt_train_muon.py 保持一致以便公平对比
    ...
    # 注意：这里的 learning_rate 是专门为单一 AdamW 优化器调好的基线学习率，
    # 数值上不同于 gpt_train_muon.py 中 AdamW 分支使用的学习率（5e-5）——
    # 这是两份脚本各自独立调参的正常差异，不是本文件的 bug。
    ...
    # 加载刚保存的参数，验证保存/加载流程可用；weights_only=True 需要 PyTorch >= 1.13
    # 才支持该关键字参数（跨版本风险，此处只标注、不修改），其作用是出于安全考虑，
