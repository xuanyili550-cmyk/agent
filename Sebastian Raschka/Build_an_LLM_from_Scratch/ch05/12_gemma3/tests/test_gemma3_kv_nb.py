...
"""
本模块是针对 `standalone-gemma3-plus-kvcache.ipynb` 这个 Jupyter Notebook 中
定义的 Gemma3（含 KV 缓存版本）模型实现所写的 pytest 测试文件。
...
注意（本次审阅发现的观察点，未做修改，仅标注上报）：
1. 第 15 行 `importlib.util.find_spec(...)` 只 `import importlib`，
...
2. 本文件名为 `test_gemma3_kv_nb.py`，加载的也是带 KV 缓存实现的
   notebook（`standalone-gemma3-plus-kvcache.ipynb`），但下面两个测试
   实际上都只做了"一次性整段前向传播"（prefill 一次性喂入全部 8 或
   context_length 个 token），并没有专门针对"增量式、多步 KV 缓存
   解码"（即逐 token 喂入、复用并增长缓存）路径进行断言。也就是说，
   KV 缓存相关的增量推理逻辑在这份测试里并未被直接覆盖到。这属于
   测试覆盖面上的观察点，而非代码中的确定性 bug，同样不做修改，
   仅在此标注、并在返回结果中上报。
"""
...
# （见上方模块 docstring 中的观察点 1：这里隐式依赖 importlib.util 已被间接导入。）
...
def import_notebook_defs():
    """
    pytest 夹具（fixture）：动态加载 `standalone-gemma3-plus-kvcache.ipynb`
    中定义的所有类和函数（如 Gemma3Model、load_weights_into_gemma 等），
...
def dummy_cfg_base():
    """
    pytest 夹具：提供一份"迷你版" Gemma3（含 KV 缓存）模型配置字典，
    用于 `test_dummy_gemma3_forward` 的快速冒烟测试。所有维度都被
...
def test_dummy_gemma3_forward(dummy_cfg_base, dummy_input, import_notebook_defs):
    """
    冒烟测试：使用迷你配置构造 Gemma3Model（KV 缓存版本），执行一次
    （不使用缓存的）整段前向传播，只验证输出张量的形状是否为
    (batch_size, seq_len, vocab_size)。

    该测试不校验具体数值是否正确，也未专门验证 KV 缓存的增量解码
    路径，只确认模型各层维度衔接无误、能够端到端跑通，属于最基础的
    "能不能跑"级别的验证。
    """
...
def test_gemma3_base_equivalence_with_transformers(import_notebook_defs):
    """
    等价性测试：验证 notebook 中从零实现的 Gemma3Model（KV 缓存版本）
    与 HuggingFace 官方 `transformers` 库中的 Gemma3ForCausalLM 在相同
    权重、相同输入下是否输出数值上一致（在给定误差范围内）。

    步骤：
    1. 用同一份小配置分别构造本地 Gemma3Model 和 HF 版 Gemma3ForCausalLM；
    2. 把 HF 模型的 state_dict 通过 `load_weights_into_gemma` 转换/加载进
       本地模型，确保两者使用完全相同的权重；
    3. 用同一批随机 token 输入两个模型（一次性整段前向，未走增量 KV
       缓存解码路径），比较输出 logits 是否在误差范围内一致。
    ...
    # 构造本地（notebook 中定义的）Gemma3 模型实现（KV 缓存版本）
